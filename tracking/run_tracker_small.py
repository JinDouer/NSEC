...
def enlarge(init_bbox,scale=2):
    init_center_x = init_bbox[0]+init_bbox[2]//2
    init_center_y = init_bbox[1]+init_bbox[3]//2
    enlarge_w = init_bbox[2]*scale
    enlarge_h = init_bbox[3]*scale
    enlarge_x = max(init_center_x - enlarge_w//2,0)
    enlarge_y = max(init_center_y - enlarge_h//2,0)
    return np.array([enlarge_x,enlarge_y,enlarge_w,enlarge_h])
def decrease(result_bb,scale=2):
    result_bb[:,0] = result_bb[:,0] + result_bb[:,2]/2
    result_bb[:,1] = result_bb[:,1] + result_bb[:,3]/2
    result_bb[:,2] = result_bb[:,2]/scale
    result_bb[:,3] = result_bb[:,3]/scale
    result_bb[:,0] = result_bb[:,0] - result_bb[:,2]/2
    result_bb[:,1] = result_bb[:,1] - result_bb[:,3]/2
    result_bb.round()
    return result_bb
...
def run_mdnet(img_list, init_bbox, gt=None, savefig_dir='', display=False):
    ...
    enlarged = False
    # Init bbox
    if init_bbox[2]*init_bbox[3] < 100:
        enlarged = True
        init_bbox = enlarge(init_bbox)
        gt = decrease(gt,0.5)   
