...
def stackList(featList):
    ...
    return pos_data
def forward_samples(model, image, samples, out_layer='conv3',train=False):
    model.eval()
    extractor = RegionExtractor(image, samples, opts['img_size'], opts['padding'], opts['batch_test'],train=train)
...
def run_mdnet(img_list, init_bbox, gt=None, savefig_dir='', display=False):
    ...
    pos_feats1 = forward_samples(model, image, pos_examples1,train=True)
    neg_feats1 = forward_samples(model, image, neg_examples1,train=True)
    ##class2:over include
    ...
    pos_feats2 = forward_samples(model, image, pos_examples2,train=True)
    neg_feats2 = forward_samples(model, image, neg_examples2,train=True)
    ##class3:part+bg
    ...
    pos_feats3 = forward_samples(model, image, pos_examples3,train=True)
    neg_feats3 = forward_samples(model, image, neg_examples3,train=True)
    ##class4:pure part
    ...
    pos_feats4 = forward_samples(model, image, pos_examples4,train=True)
    neg_feats4 = forward_samples(model, image, neg_examples4,train=True)
    ...
    # Extract pos/neg features
    pos_feats = forward_samples(model, image, pos_examples,train=True)
    neg_feats = forward_samples(model, image, neg_examples,train=True)
    feat_dim = pos_feats.size(-1)
